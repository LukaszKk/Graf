...
void rysuj(void)
{
	...
	glDrawArrays(GL_QUADS, 0, 6); // Zaczynamy od 0 i rysujemy wszystkie wierzcho│ki
	...
	glUniformMatrix4fv(MVP_id2, 1, GL_FALSE, &(MVP[0][0]));	   // wyślij tablicę mv do lokalizacji "MV", która jest typu mat4	

	glBindVertexArray(VAO[2]);
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);

	glBindTexture(GL_TEXTURE_2D, texid2);
	glUniform1i(sampler_id2, 0);
...
int main(int argc, char **argv)
{
	...
	texid2 = WczytajTeksture("3.bmp");
	sampler_id = glGetUniformLocation(programID, "myTextureSampler");
	sampler_id2 = glGetUniformLocation(programID2, "myTexture");
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(textureCoord2), textureCoord2, GL_STATIC_DRAW);
